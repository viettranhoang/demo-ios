...
struct RecipeForm: View {
    ...
    init(mode: Mode) {
        ...
        case .add:
            title = "Add Recipe"
            _name = .init(initialValue: "Na")
            _summary = .init(initialValue: "Sum")
            _serving = .init(initialValue: 1)
            _time = .init(initialValue: 5)
            _instructions = .init(initialValue: "In")
    ...
    @Query private var ingredientss: [RecipeIngredient]
        ...
        Section("Ingredients") {
            if recipeIngredients.isEmpty {
                ...
                ForEach(recipeIngredients) { ingredient in
                    ...
                    if let ingre = ingredient.ingredient {
